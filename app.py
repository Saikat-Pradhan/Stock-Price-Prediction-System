...
# Define the default stock ticker and date range
start = '2010-01-01'
end = '2019-12-31'
...
companies = ['Apple', 'State Bank of India', 'Google', 'Microsoft', 'Amazon', 'Tesla']

input = st.selectbox(
    'Select Your Company',
    ['-- Select here --'] + companies,
    index=0
)

if input != '-- Select here --':
    if input == 'Apple':
        ticker = 'AAPL'
    ...
    start = st.date_input('Start Date', value=pd.to_datetime(start))
    end = st.date_input('End Date', value=pd.to_datetime(end))
    ...
    if ticker:
        ...
        # Describe Data
        st.subheader(f"{input} Stock Price Data ({start} to {end})")
